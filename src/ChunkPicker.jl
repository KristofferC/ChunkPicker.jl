...
"""
    smart_chunks(n::Integer) -> Vector{Int}

The default chunk-size candidates for an input of length `n`: the sizes that
ever win, measured on a benchmark grid of 6 function families x 20 input
sizes x chunks 1:16 x simd on/off, on AVX2 (Arrow Lake), AVX-512 (Zen 4) and
NEON (Apple M4) — see `benchmark/RESULTS.md`. For `n <= 4` everything is
included; above that:
...
- `n` itself while `n <= 16`: the whole Hessian in one evaluation;
- `cld(n, 2)` and `cld(n, 3)`: fewest evaluations per dual size;
- divisors of `n` in `4:16`: no padded trailing chunk.

Across the measured grid the best candidate in this set is within 2% of the
exhaustive optimum in 99% of cases (worst 9%), with ~40% fewer candidates
than the previous dense sweep. Pass `chunks = :all` to `pick_chunk` for the
exhaustive sweep instead.
"""
function smart_chunks(n::Integer)
    n <= 0 && return Int[]
    n <= 4 && return collect(1:n)
    chunks = Int[c for c in (2, 3, 4, 6, 8, 12, 16) if c < n]
    n <= 16 && push!(chunks, n)
    for k in 2:3
        c = cld(n, k)
        c < n && c <= 16 && push!(chunks, c)
    end
    for d in 4:16
...
function _resolve_chunks(chunks, n::Int, op::Symbol)
    chunks === :smart && return smart_chunks(n)
    if chunks === :all
        op === :hessian || op === :hvp || return collect(1:min(n, 32))
        full = collect(1:min(n, 16))
        16 < n <= 20 && push!(full, n)
        return full
    end
...
function pick_chunk(
    ...
    candidates = [(N, :chunk, false) for N in _resolve_chunks(chunks, length(x), op)]
